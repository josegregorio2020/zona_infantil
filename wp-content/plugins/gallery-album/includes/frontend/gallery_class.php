...
class wpdevart_gallery_theme {
    ...
    public function load_image_info() {
        ...
        $obj = new stdClass();
        $obj->gallery_current_index = intval($_GET['gallery_current_index']);
        $obj->album_current_index = intval($_GET['album_current_index']);
        $obj->start = intval($_GET['start']);
        $obj->limit = intval($_GET['limit']);
        $output = '';
    ...
    public function shortcode($atts) {
		...
		<script>
			...
			///////////////////////////       views gallery:ALL_, ALL, album, imaje
				...
				if('$gallery_id' != 'all'){if('$gallery_id' != 'all_'){if('$album_id' == 'all'){
					Glr_" . self::$prefix . ".main_conteiner.style.height='1px';  //for IE 11
					Glr_" . self::$prefix . ".gall_resizeble = 1;
