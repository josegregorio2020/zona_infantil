...
// WP-Optimize advanced-cache.php (written by version: 4.2.3) (homeurl: https://zonainfantil.ibague.gov.co/) (abspath: /var/www/zonainfantil/public_html/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/var/www/zonainfantil/public_html/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://zonainfantil.ibague.gov.co/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/var/www/zonainfantil/public_html/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-zonainfantil.ibague.gov.co.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-zonainfantil.ibague.gov.co.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-zonainfantil.ibague.gov.co.php');
}
